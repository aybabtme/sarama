...
// Consumer implements sarama's Consumer interface for testing
// purposes. Before you can start consuming from this consumer, you
// have to register topic/partitions using ExpectConsumePartition, and
// set expectations on them.
...
// NewConsumer returns a new mock Consumer instance. The t argument
// should be the *testing.T instance of your test method. An error
// will be written to it if an expectation is violated. The config
// argument is currently unused and can be set to nil.
...
// ConsumePartition implements the ConsumePartition method from the
// sarama.Consumer interface. Before you can start consuming a
// partition, you have to set expectations on it using
// ExpectConsumePartition. You can only consume a partition once per
// consumer.
...
// Partitions returns the list of parititons for the given topic, as
// registered with SetMetadata
...
// Close implements the Close method from the sarama.Consumer
// interface. It will close all registered PartitionConsumer
// instances.
...
// ExpectConsumePartition will register a topic/partition, so you can
// set expectations on it. The registered PartitionConsumer will be
// returned, so you can set expectations on it using method chanining.
// Once a topic/partition is registered, you are expected to start
// consuming it using ConsumePartition. If that doesn't happen, an
// error will be written to the error reporter once the mock consumer
// is closed. It will also expect that the
...
// PartitionConsumer implements sarama's PartitionConsumer interface
// for testing purposes. It is returned by the mock Consumers
// ConsumePartitionMethod, but only if it is registered first using
// the Consumer's ExpectConsumePartition method. Before consuming the
// Errors and Messages channel, you should specify what values will be
// provided on these channels using YieldMessage and YieldError.
...
// AsyncClose implements the AsyncClose method from the
// sarama.PartitionConsumer interface.
...
// Close implements the Close method from the sarama.PartitionConsumer
// interface. It will verify whether the partition consumer was
// actually started.
...
// Errors implements the Errors method from the
// sarama.PartitionConsumer interface.
func (pc *PartitionConsumer) Errors() <-chan *sarama.ConsumerError {
	return pc.errors
}

// Messages implements the Messages method from the
// sarama.PartitionConsumer interface.
...
// YieldMessage will yield a messages Messages channel of this
// partition consumer when it is consumed. By default, the mock
// consumer will not verify whether this message was consumed from the
// Messages channel, because there are legitimate reasons forthis not
// to happen. ou can call ExpectMessagesDrainedOnClose so it will
...
// YieldError will yield an error on the Errors channel of this
// partition consumer when it is consumed. By default, the mock
// consumer will not verify whether this error was consumed from the
// Errors channel, because there are legitimate reasons for this not
// to happen. You can call ExpectErrorsDrainedOnClose so it will
// verify that the channel is empty on close.
func (pc *PartitionConsumer) YieldError(err error) {
	pc.expectations <- &consumerExpectation{Err: err}
}

// ExpectMessagesDrainedOnClose sets an expectation on the partition
// consumer that the messages channel will be fully drained when Close
// is called. If this expectation is not met, an error is reported to
// the error reporter.
func (pc *PartitionConsumer) ExpectMessagesDrainedOnClose() {
	pc.messagesShouldBeDrained = true
}

// ExpectErrorsDrainedOnClose sets an expectation on the partition
// consumer that the errors channel will be fully drained when Close
// is called. If this expectation is not met, an error is reported to
// the error reporter.
