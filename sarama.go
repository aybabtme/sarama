...
// connection management events to. By default it is set to discard
// all log messages via ioutil.Discard, but you can set it to redirect
// wherever you want.
...
// PanicHandler is called for recovering from panics spawned
// internally to the library (and thus not recoverable by the caller's
// goroutine). Defaults to nil, which means panics are not recovered.
var PanicHandler func(interface{})

// MaxRequestSize is the maximum size (in bytes) of any request that
// Sarama will attempt to send. Trying to send a request larger than
// this will result in an PacketEncodingError. The default of 100 MiB
// is aligned with Kafka's default `socket.request.max.bytes`, which
// is the largest request the broker will attempt to process.
var MaxRequestSize int32 = 100 * 1024 * 1024

// MaxResponseSize is the maximum size (in bytes) of any response that
// Sarama will attempt to parse. If a broker returns a response
// message larger than this value, Sarama will return a
// PacketDecodingError to protect the client from running out of
// memory. Please note that brokers do not have any natural limit on
// the size of responses they send. In particular, they can send
// arbitrarily large fetch responses to consumers (see
